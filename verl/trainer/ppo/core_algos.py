...
@register_adv_est("MEDS")
def compute_grpo_cluster_score_minlog_both_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    config: Optional[AlgoConfig] = None,
    cluster_size_per_sample: Optional[torch.Tensor | np.ndarray] = None,
    cluster_penalty_correctness: Optional[torch.Tensor | np.ndarray] = None,
    **kwargs,
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    GRPO + cluster penalty (score field, both versions):
        For incorrect samples: minlog penalty is applied (similar to compute_grpo_cluster_score_minlog_outcome_advantage)
        For correct samples: sign-preserving minlog is used (similar to compute_grpo_cluster_score_minlog_sign_preserving_outcome_advantage)

    Correctness/incorrectness is specified by cluster_penalty_correctness:
        1 indicates correct, 0 indicates incorrect, other values indicate unknown (no penalty is applied).
    """
    penalty_coef = config.algorithm.get("penalty_coef", 0.1)

    scores = token_level_rewards.sum(dim=-1)  # (bs,)

    if cluster_size_per_sample is not None and cluster_penalty_correctness is not None:
        if isinstance(cluster_size_per_sample, np.ndarray):
            cluster_size_per_sample = torch.from_numpy(cluster_size_per_sample).to(
                device=scores.device, dtype=scores.dtype
            )
        elif isinstance(cluster_size_per_sample, torch.Tensor):
            cluster_size_per_sample = cluster_size_per_sample.to(
                device=scores.device, dtype=scores.dtype
            )
        else:
            raise TypeError(
                f"cluster_size_per_sample must be torch.Tensor or np.ndarray, got {type(cluster_size_per_sample)}"
            )

        if isinstance(cluster_penalty_correctness, np.ndarray):
            cluster_penalty_correctness = torch.from_numpy(cluster_penalty_correctness).to(
                device=scores.device
            )
        elif isinstance(cluster_penalty_correctness, torch.Tensor):
            cluster_penalty_correctness = cluster_penalty_correctness.to(device=scores.device)
        else:
            raise TypeError(
                f"cluster_penalty_correctness must be torch.Tensor or np.ndarray, got {type(cluster_penalty_correctness)}"
            )

        wrong_mask = cluster_penalty_correctness == 0
        right_mask = cluster_penalty_correctness == 1


        C = 2.0
        base_penalty = -penalty_coef * torch.clamp(
            torch.log(cluster_size_per_sample + 1),
            max=C,
        )

        if wrong_mask.any():
            scores[wrong_mask] = scores[wrong_mask] + base_penalty[wrong_mask]

        if right_mask.any():
            scores[right_mask] = scores[right_mask] / (1 - base_penalty[right_mask])
