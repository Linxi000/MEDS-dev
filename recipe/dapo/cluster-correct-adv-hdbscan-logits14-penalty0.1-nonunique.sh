...
# 0
exp_name='cluster-correct-adv-hdbscan-logits14-penalty0.1-nonunique_v2'

# 1 改成相应的处理函数
adv_estimator=grpo_cluster_adv_both
# 2 改成相应的hdbscan版本
...
# 3
cluster_penalty_target="right" # "wrong", "right", "both", "none"
noise_forget_epochs=0
...
CKPTS_DIR=${CKPTS_DIR:-"/inspire/qb-ilm/project/exploration-topic/public/yfgao/ckpts/${project_name}/${exp_name}"}
...
RUNTIME_ENV_FILE="/inspire/qb-ilm/project/exploration-topic/public/yliu/yfgao/runtime_env/${RUN_NAME}.yaml"
WANDB_BASE_DIR=${WANDB_BASE_DIR:-"/inspire/qb-ilm/project/exploration-topic/public/yfgao/wandb"}
...
ray job submit --no-wait --runtime-env="${RUNTIME_ENV_FILE}" \
    ...
    +trainer.clustering.use_pca=${use_pca} \
    +trainer.clustering.pca_components=${pca_components} \
    +trainer.clustering.pca_variance_ratio=${pca_variance_ratio} \
    +trainer.clustering.cluster_penalty_target=${cluster_penalty_target} \
    +trainer.clustering.noise_forget_epochs=${noise_forget_epochs} \
